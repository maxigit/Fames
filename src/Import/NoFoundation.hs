...
transactionIcons eType = case eType of
  ...
  ST_SALESINVOICE -> [ IGL, IInLeft, ICustomer ]
  ST_CUSTCREDIT ->   [ IGL, IOutRight, ICustomer ]
  ST_CUSTPAYMENT -> [ IBank, IInLeft, ICustomer ]
  ST_CUSTDELIVERY -> [ IStock, IOutRight, ICustomer ]
  ...
  ST_SALESORDER -> [ IOrder, IInLeft,  ICustomer ]
  ST_SALESQUOTE -> [ IOrder, IInLeft,  ICustomer ]
  ST_COSTUPDATE -> [  ]
  ST_DIMENSION -> [  ]
