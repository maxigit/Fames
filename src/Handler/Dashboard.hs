...
top20ItemMonth f begin rupture = do
  ...
  let param = f ReportParam{..}
      ...
      rpCategoryToFilter = Nothing
      rpCategoryFilter = Nothing
      rpSkuFilter = Nothing -- Just (LikeFilter "ML1_-A_2-BLK")
      rpShowInactive = True
      rpPanelRupture = emptyRupture
      rpBand = emptyRupture
...
top100ItemYear which rupture = do
  ...
  let param = ReportParam{..}
      ...
      rpTo = Just today
      rpPeriod' = Just PFSlidingYearFrom
      rpNumberOfPeriods = Nothing -- Just 2
      rpCategoryToFilter = Nothing
      rpCategoryFilter = Nothing
      rpSkuFilter = Nothing -- Just (LikeFilter "ML1_-A_2-BLK")
      rpShowInactive = True
...
getDForecastDetailedR pathm = do
  ...
  let names  = take 1 ["style", "base", "random-base", "shape", "random-shape", "random-10", "random-100", "random-p4", "dpd-origin"] :: [Text]
