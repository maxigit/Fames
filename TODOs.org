...
** Today [0/3] 
   - [ ] Finish margin
     - [ ] add max, total and average  option to residual
     - [ ]  fixes bug, showing an extra "level" when residual
   - [ ] memoize TranQP virtual fieed
   - [ ] add QPType as virtual field
     - [ ] date `mod` year
     - [ ]  Remove semigroup constraints on NMap
