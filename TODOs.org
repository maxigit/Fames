* TODO Sales Order report :  [2/4]
  - [-] Sort QSales, how do we it ?
    - [X] Have a special entry SalesOrder
    - [ ] and map it either to Sales or Purchase depending on paramters
    - [ ] Display in summary
      - [ ]  filter based on what's is present in the all map
  - [X] sort speed problem
    +- [ ]  use normal regex+
    +- [ ]  don't load map+
    - [X]  break loading of map
  - [X]  add order category to order
  - [ ] param to load order or not
