* TODO Sales Order report :  [3/5]
  - [X] Bug: not displaying summary (total) column as above
  - [ ] Bug: stock_id used to filter sku not working (for sales_orders)
  - [ ] Bug: category join not working
...
Forecast are purchases, if we want to see if the forecast is enougth to cope with the sales (we forecast what to buy)
